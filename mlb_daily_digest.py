...
DIV_ORDER = [201, 202, 200, 204, 205, 203]  # East → Central → West, AL then NL

def logo(abbr, sz=28):
    return (f"<img src='https://a.espncdn.com/i/teamlogos/mlb/500/{abbr.lower()}.png' "
            f"width='{sz}' height='{sz}' style='vertical-align:middle;object-fit:contain;display:inline-block'>")
...
def get_games(d):
    r = requests.get(
        f"{BASE}/schedule?sportId=1&date={d}&hydrate=linescore,decisions",
        timeout=10
    )
...
def render_hero_card(game, my_id, css_class, label):
    ...
    hid   = home["team"]["id"]
    habbr = home["team"].get("abbreviation", "")
    aabbr = away["team"].get("abbreviation", "")
...
# ══ SECTION 6: Tonight's Projected Starters ══════════════════════════
st.markdown('<div class="gap"></div>', unsafe_allow_html=True)
