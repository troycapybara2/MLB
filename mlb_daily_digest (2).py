...
def logo(abbr, sz=28):
    ...
    if not tid:
        return f"<span style='font-size:10px;color:#33333f'>{abbr}</span>"
    url = f"https://www.mlbstatic.com/team-logos/{tid}.svg"
    return (f"<img src='{url}' width='{sz}' height='{sz}' "
            f"style='vertical-align:middle;object-fit:contain;display:inline-block'>")
...
.stApp { background: #0a0a10 !important; }
...
.game-row.phi-game { background: #14080a; border-left: 3px solid #e81828; }
.game-row.nyy-game { background: #08080f; border-left: 3px solid #4d7bc4; }
...
.insight-icon { font-size: 1rem; line-height: 1.5; flex-shrink: 0; margin-top: 0.05rem; }
.insight-text {
    font-size: 0.85rem;
    color: #a0a0bc;
    line-height: 1.55;
}
.insight-text b { color: #ccccd8; font-weight: 600; }
...
def generate_batting_insights():
    ...
    for p in players[:30]:
        ...
        # Hitting streak
        if streak >= 15:
            entries.append({"text": f"<b>{name}</b> ({team}) is riding a <b>{streak}-game hitting streak</b> — one of the longest active in the majors", "pri": streak + 90, "icon": "🔥"})
        elif streak >= 10:
            entries.append({"text": f"<b>{name}</b> ({team}) has hit safely in <b>{streak} consecutive games</b>", "pri": streak + 50, "icon": "🔥"})
        elif streak >= 7:
            entries.append({"text": f"<b>{name}</b> ({team}) has reached base via hit in <b>{streak} straight games</b>", "pri": streak + 20, "icon": "🔥"})

        # HR burst (last 7)
        if l7["hrs"] >= 5:
            entries.append({"text": f"<b>{name}</b> ({team}) has been on a power surge — <b>{l7['hrs']} HRs in {l7['g']} games</b>", "pri": l7["hrs"] * 20, "icon": "💣"})
        elif l7["hrs"] >= 3:
            entries.append({"text": f"<b>{name}</b> ({team}) has <b>{l7['hrs']} HRs</b> over the last {l7['g']} games", "pri": l7["hrs"] * 14, "icon": "💣"})

        # HR burst (last 14 only if not already flagged last 7)
        if l14["hrs"] >= 8 and l7["hrs"] < 3:
            entries.append({"text": f"<b>{name}</b> ({team}) has <b>{l14['hrs']} HRs</b> over the last 14 games", "pri": l14["hrs"] * 7, "icon": "💣"})

        # RBI burst
        if l7["rbi"] >= 11:
            entries.append({"text": f"<b>{name}</b> ({team}) has been a run-producing machine — <b>{l7['rbi']} RBIs</b> over the last {l7['g']} games", "pri": l7["rbi"] * 6, "icon": "💥"})
        elif l7["rbi"] >= 7:
            entries.append({"text": f"<b>{name}</b> ({team}) has driven in <b>{l7['rbi']} runs</b> over the last {l7['g']} games", "pri": l7["rbi"] * 4, "icon": "💥"})

        # Hot bat (meaningful AB sample)
        if l7["ab"] >= 22 and l7["ba"] >= 0.400:
            entries.append({"text": f"<b>{name}</b> ({team}) is locked in — <b>.{int(l7['ba']*1000):03d}</b> over the last {l7['g']} games ({l7['hits']}-for-{l7['ab']})", "pri": int(l7["ba"] * 560), "icon": "🎯"})
        elif l7["ab"] >= 15 and l7["ba"] >= 0.467:
            entries.append({"text": f"<b>{name}</b> ({team}) is scorching right now — <b>.{int(l7['ba']*1000):03d}</b> over the last {l7['g']} games", "pri": int(l7["ba"] * 650), "icon": "🎯"})
        ...
        if p.get("value") and p in players and gp >= 50:
            try:
                ...
                if pace >= 50:
                    entries.append({"text": f"<b>{name}</b> ({team}) leads the majors with {s_hrs} HRs and is on pace for <b>{pace} this season</b>", "pri": pace + 35, "icon": "📈"})
                elif pace >= 43:
                    entries.append({"text": f"<b>{name}</b> ({team}) is on pace for <b>{pace} HRs</b> — currently at {s_hrs} on the year", "pri": pace + 20, "icon": "📈"})
            except Exception:
                pass
    ...
    for tracked_id, opp_id, label in ((PHI_ID, phi_opp, "PHI"), (NYY_ID, nyy_opp, "NYY")):
        ...
        for p in team_players.get(label, [])[:5]:
            ...
            ba = round(hits / ab, 3)
            if ba >= 0.350:
                raw.append({"text": f"<b>{p['name']}</b> ({label}) is batting <b>.{int(ba*1000):03d}</b> ({hits}-for-{ab}) vs {opp_abbr} — tonight's opponent", "pri": int(ba * 320) + 65, "icon": "🎯", "pid": pid})
            elif hrs >= 3:
                raw.append({"text": f"<b>{p['name']}</b> ({label}) has <b>{hrs} career HRs</b> against tonight's opponent ({opp_abbr})", "pri": hrs * 12 + 45, "icon": "💣", "pid": pid})
